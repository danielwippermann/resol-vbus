...
const { TcpConnection, TcpConnectionEndpoint } = require('./resol-vbus');
    ...
    describe('#constructor', () => {
        it('should have reasonable defaults', () => {
            ...
            expectOwnPropertyNamesToEqual(
                ep,
                ['port', 'verifyViaTag', 'password', 'verifyPassword', 'channels', 'verifyChannel', 'verifyDataMode'],
                [
                    // base class related
                    '_events',
                    '_eventsCount',
    ...
    describe('integration tests', () => {
        ...
        it('should work correctly', async () => {
            await runTestableEndpointAndConnection(async ({ ep, conn }) => {
                ...
                conn.channelListCallback = async (channels) => channels[0];
        ...
        it('should work correctly with a rejecting viaTag verifier', async () => {
            await runTestableEndpointAndSocket(async ({ ep, socket, read }) => {
                const onConnectionAttemptFailed = vi.fn();
                ...
                expectOwnPropertyNamesToEqual(ev1, ['ip', 'port', 'family', 'error']);
        ...
        it('should work correctly for mismatching passwords', async () => {
            ...
            try {
                ...
                await expect(() => conn.connect()).rejects.toThrow(
                    'Remote side responded with "-ERROR: \\"Error: Password mismatch\\""',
                );
            } finally {
                if (conn) {
                    conn.disconnect();
        ...
        it('should work correctly with a rejecting password verifier', async () => {
            await runTestableEndpointAndSocket(async ({ ep, socket, read }) => {
                ...
                ep.verifyPassword = async (viaTag) => {
        ...
        it('should work correctly for mismatching channels', async () => {
            ...
            try {
                ...
                await expect(() => conn.connect()).rejects.toThrow(
                    'Remote side responded with "-ERROR: \\"Error: Channel not available\\""',
                );
            } finally {
                if (conn) {
                    conn.disconnect();
        ...
        it('should work correctly with a rejecting channel verifier', async () => {
            await runTestableEndpointAndSocket(async ({ ep, socket, read }) => {
                ...
                ep.verifyChannel = async (indexString, index, channel) => {
