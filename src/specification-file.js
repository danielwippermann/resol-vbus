...
const calcCrc16 = (buffer, start, end) => {
    let checksum = 0xffff;
...
class SpecificationFile {
    ...
    _generateSpecificationData(language) {
        ...
        const types = this.packetTemplates.reduce((memo, pt) => {
            return pt.fields.reduce((memo, ptf) => {
                ...
                typeId += sprintf('%.' + precision + 'f', 10 ** -precision).replace(/[^0-9]/g, '_');
                typeId += '_' + unitCode;
        ...
        const getRawValueFunctions = this.packetTemplates.reduce((memo, pt) => {
            return pt.fields.reduce((memo, ptf) => {
                const funcId = sprintf(
                    '_%04X_%04X_%04X_%s',
                    pt.destinationAddress,
                    pt.sourceAddress,
                    pt.command,
                    ptf.id,
                );

                memo[funcId] = (buffer, start, end) => this.getRawValue(pt, ptf, buffer, start, end);
        ...
        const setRawValueFunctions = this.packetTemplates.reduce((memo, pt) => {
            return pt.fields.reduce((memo, ptf) => {
                ...
                memo[funcId] = (newValue, buffer, start, end) =>
                    this.setRawValue(pt, ptf, newValue, buffer, start, end);
        ...
        const getDeviceSpecification = (selfAddress, peerAddress) => {
            ...
            if (deviceSpecCache.has(deviceSpecId)) {
                ...
                const dt = this.deviceTemplates.find((dt) => {
        ...
        const getPacketSpecification = (destinationAddress, sourceAddress, command) => {
            ...
            if (packetSpecCache.has(packetSpecId)) {
                ...
                const pt = this.packetTemplates.find((pt) => {
    ...
    _parseBuffer(buffer) {
        const sliceBlock = (name, offset, size) => {
        ...
        const processBlockTable = (blockName, tableCount, tableOffset, blockSize, cb) => {
        ...
        processBlockTable(
            'TEXT',
            specificationBlock.textCount,
            specificationBlock.textTableOffset,
            0x04,
            (textBuffer) => {
                const textBlock = {
        ...
        processBlockTable(
            'LOCALIZEDTEXT',
            specificationBlock.localizedTextCount,
            specificationBlock.localizedTextTableOffset,
            0x0c,
            (locTextBuffer) => {
                const locTextBlock = {
        ...
        processBlockTable(
            'UNIT',
            specificationBlock.unitCount,
            specificationBlock.unitTableOffset,
            0x10,
            (unitBuffer) => {
                const unitBlock = {
                ...
                units.push({
                    ...
                    unitFamily: this.unitFamilyById[unitBlock.unitFamilyId] || null,
        ...
        processBlockTable(
            'DEVICETEMPLATE',
            specificationBlock.deviceTemplateCount,
            specificationBlock.deviceTemplateTableOffset,
            0x0c,
            (dtBuffer) => {
                const dtBlock = {
        ...
        processBlockTable(
            'PACKETTEMPLATE',
            specificationBlock.packetTemplateCount,
            specificationBlock.packetTemplateTableOffset,
            0x14,
            (ptBuffer) => {
                const ptBlock = {
                ...
                processBlockTable(
                    ...
                    (ptfBuffer) => {
                        ...
                        fields.push({
                            ...
                            type: this.typeById[ptfBlock.typeId],
                            parts,
                        });
                    },
                );
    ...
    getRawValue(pt, ptf, buffer, start, end) {
        ...
        if (ptf.precision > 0) {
            rawValue = rawValue * 10 ** -ptf.precision;
        }
        if (!valid) {
            rawValue = null;
        }
    ...
    setRawValue(pt, ptf, newValue, buffer, start, end) {
        if (ptf.precision > 0) {
            newValue = newValue * 10 ** ptf.precision;
        }
